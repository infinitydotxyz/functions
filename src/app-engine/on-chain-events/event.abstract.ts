...
export abstract class AbstractEvent<T> {
  protected _addresses: Set<string>;
  ...
  public get eventFilters(): EventFilter[] {
    return [...this._addresses.values()].map((address) => {
      return {
        address,
        topics: this._topics
      };
  ...
  constructor(
    ...
    this._addresses = new Set(_addresses);
  }

  protected matches(event: ethers.providers.Log, baseParams: BaseParams): boolean {
    if (!this._addresses.has(baseParams.address)) {
      return false;
    }
    return event.topics[0] === this._topic && event.topics.length === this._numTopics;
  }

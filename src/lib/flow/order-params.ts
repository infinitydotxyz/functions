...
export class OrderParams implements Types.OrderInput {
  ...
  get trustedExecution(): string | undefined {
    return this._params.constraints[7];
  }

  set trustedExecution(trustedExecution: string | undefined) {
    if (this._params.constraints.length >= 8) {
      if (typeof trustedExecution === 'string') {
        this._params.constraints[7] = trustedExecution;
      } else {
        this._params.constraints.pop();
      }
    } else if (this._params.constraints.length === 7) {
      if (typeof trustedExecution === 'string') {
        this._params.constraints.push(trustedExecution);
      }
    } else {
      throw new Error('Invalid order constraints');
    }
  }
  ...
  protected set sig(signature: string | Signature | { v: number; r: string; s: string }) {
    let sig: string;
  ...
  async setSig(signature: string | Signature | { v: number; r: string; s: string }) {
    let sig: string;
    const complication = getComplication(this.chainId, this.complication);
    if (typeof signature === 'string') {
      sig = signature;
    ...
    await complication.verifySignature(sig, this.getInternalOrder(this.params));
    this._sig = sig;
  }
  ...
  constructor(protected _chainId: number, params: Types.OrderInput | Types.InternalOrder | Types.SignedOrder) {
    try {
  ...
  public getInternalOrder(params: Types.OrderInput): Types.InternalOrder {
    ...
    if (params.trustedExecution != null) {
      internalOrder.constraints.push(params.trustedExecution);
    }
...
export const normalize = (order: Types.OrderInput | Types.InternalOrder | Types.SignedOrder): Types.OrderInput => {

...
import { QuerySnap, CollRef, DocRef } from '../../firestore/types';
...
export class ReservoirOrderStatusEventProcessor extends FirestoreBatchEventProcessor<FirestoreOrderEvent> {
  ...
  protected async _processEvents(eventsSnap: QuerySnap<Reservoir.OrderEvents.Types.FirestoreOrderEvent>, txn: FirebaseFirestore.Transaction, eventsRef: CollRef<Reservoir.OrderEvents.Types.FirestoreOrderEvent>): Promise<void> {
    ...
    const descendingEvents = eventsSnap.docs.map((item) => { 
        return {
            data: item.data(),
            ref: item.ref
        }
  ...
  protected _getReservoirOrder(id: string, chainId: ChainId, isSellOrder: boolean) {

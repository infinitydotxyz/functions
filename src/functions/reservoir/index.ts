...
import { syncSaleEvents as _syncSaleEvents } from './sync-sale-events';
...
export const syncSaleEvents = functions
  .region(config.firebase.region)
  .runWith({ timeoutSeconds: 530, maxInstances: 1 })
  .pubsub.schedule('every 9 minutes')
  .onRun(async () => {
    const db = getDb();
    const stopIn = 530 * 1000;
    await _syncSaleEvents(db, stopIn, { pollInterval: 10_000, delay: 0 });
  });

...
export async function updateUserStake(user: string, event: StakerEvents, db: FirebaseFirestore.Firestore): Promise<{ userStake: UserStake, totalCuratedVotes: number }> {
  const userRef = db.collection(firestoreConstants.USERS_COLL).doc(user);

  const userStakeAndVotes = await db.runTransaction<{ userStake: UserStake, totalCuratedVotes: number }>(async (txn) => {

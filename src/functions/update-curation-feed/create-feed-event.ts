...
export async function createFeedEventForLedgerEvent(
  ...
  await db.runTransaction(async (txn) => {
    ...
    if (
      (ledgerEvent &&
        'isFeedUpdated' in ledgerEvent &&
        ledgerEvent.isFeedUpdated === false &&
        ledgerEvent.discriminator === CurationLedgerEvent.VotesAdded) ||
      ledgerEvent?.discriminator === CurationLedgerEvent.VotesRemoved
    ) {

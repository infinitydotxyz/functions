...
export async function markExpiredOrdersInvalid(): Promise<void> {
  ...
  const validActiveOrders = orders.where(
    "orderStatus",
    "==",
    OBOrderStatus.ValidActive
  );

  const expiredValidActiveOrders = validActiveOrders.where(
    "endTimeMs",
    "<=",
    Date.now()
  );

  const stream = expiredValidActiveOrders.stream() as AsyncIterable<

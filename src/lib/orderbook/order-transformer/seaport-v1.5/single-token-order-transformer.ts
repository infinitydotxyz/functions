import { SeaportBase, SeaportV15 } from '@reservoir0x/sdk';

import { ErrorCode } from '../../errors/error-code';
import { OrderError } from '../../errors/order.error';
import { TransformationResult } from '../types';
import { SeaportV15OrderTransformer } from './base-order-transformer';

export class SingleTokenOrderTransformer extends SeaportV15OrderTransformer {
  ...
  public transform(): Promise<TransformationResult<SeaportV15.Order>> {
    return Promise.resolve({
      isNative: false,
      sourceOrder: this._order,
      flowOrder: this.getFlowOrder(),
      getSourceTxn: async (timestamp: number, from: string) => {
        const seaport = new SeaportV15.Exchange(this.chainId);

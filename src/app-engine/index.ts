...
import { syncFlaggedTokenEvents } from '@/lib/reservoir/flagged-tokens/sync-flagged-token-events';
...
const processes = {
  ...
  syncFlagEvents: {
    enabled: true,
    start: () => {
      console.log('Starting flag sync');
      syncFlaggedTokenEvents(db, null).catch((err) => {
        console.error(`Failed to run sync flag events`, err);
      });
    }
  }
};

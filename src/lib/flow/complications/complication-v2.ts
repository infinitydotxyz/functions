...
export class ComplicationV2 implements Complication {
  ...
  async verifySignature(sig: string, params: Types.InternalOrder, provider?: Provider): Promise<void> {
    const { types, value, domain } = this.getSignatureData(params);
    ...
    try {
      ...
      const isBulkSignature =
        actualSignatureLength < 837 && actualSignatureLength > 98 && (actualSignatureLength - 67) % 32 < 2;
      if (isBulkSignature) {
        ...
        const key = bn('0x' + proofAndSignature.slice(2 + signatureLength, 2 + signatureLength + 6)).toNumber();
      ...
      const result = await new Contract(params.signer, iface, provider).isValidSignature(eip712Hash, sig);
